...
foreach($file as $key => $value)
{
	...
	$Output .= '
			...
			<td class="name"><a href="'.$value['Path']. DIRECTORY_SEPARATOR .$value['fichier'].'" title="'.$value['fichier'].'">'.detect_new($value['temps'],$LastLoad).give_etat($value['fichier']).clean_name($value['fichier'], $value['Path'], $way.$name).'</a></td>			
			<td width="25px">'.$stream.'</td>	
			<td width="25px">'.detect_allocine($value['fichier'], $value['Path'], $value['code_cat']).'</td>
			<td width="70px" class="small">'.$value['taille'].'</td>
			<td width="90px" class="small">'.date('d/m à H:i', $value['temps']).'</td>
			<td width="25px"><a title="'.$value['infovote'].'" style="cursor:help;">'.$value['moyenne'].'</a></td>
		';
